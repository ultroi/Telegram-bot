...
def play(message):
    ...
    if message.chat.id not in game_data:
        game_data[message.chat.id] = {
            ...
            'best_of': best_of,
            'scores': {},
            'player_choices': {},  # Dictionary for player choices
        ...
        if game_data[message.chat.id]['players'] >= max_players:
            bot.send_message(message.chat.id, "The game is full. You cannot join.")
            return
...
def start_auto_start_timer(chat_id):
    if 'auto_start' in game_data[chat_id] and game_data[chat_id]['auto_start']:
...
def handle_choice(call):
    ...
    if user_id in game_data[chat_id]['player_choices'] and game_data[chat_id]['player_choices'][user_id] is not None:
        bot.send_message(chat_id, "You've already made your choice.")
        return

    game_data[chat_id]['player_choices'][user_id] = user_choice
    bot.send_message(chat_id, f"{call.from_user.first_name} chose {user_choice}.")

    if len([choice for choice in game_data[chat_id]['player_choices'].values() if choice is not None]) == game_data[chat_id]['players']:
        ...
        evaluate_choices(chat_id)

def evaluate_choices(chat_id):
    choices = game_data[chat_id]['player_choices']
    bot_choice = game_data[chat_id].get('bot_choice', None)

    if not bot_choice:
        bot_choice = random.choice(['rock', 'paper', 'scissors'])

    scores = {player_id: 0 for player_id in choices}
    ties = []

    for player_id, choice in choices.items():
        if choice == bot_choice:
            scores[player_id] = 0
            ties.append(player_id)
    ...
    elif winner_score == 0 and len(ties) > 1:
        bot.send_message(chat_id, "It's a tie between the players!")
    else:
        bot.send_message(chat_id, "The bot wins the round!")

    if max(game_data[chat_id]['scores'].values(), default=0) >= game_data[chat_id]['best_of']:
        winner_id = max(game_data[chat_id]['scores'], key=game_data[chat_id]['scores'].get)
        winner_name = bot.get_chat_member(chat_id, winner_id).user.first_name
        bot.send_message(chat_id, f"{winner_name} wins the game!")
